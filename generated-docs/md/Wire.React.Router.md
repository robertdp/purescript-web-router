...
``` purescript
makeRouter :: forall route f. Foldable f => { decode :: String -> f route, encode :: route -> String, initial :: route, interface :: PushStateInterface, onRouteChange :: route -> Router route Transitioning Resolved Unit } -> Effect { navigate :: route -> Effect Unit, redirect :: route -> Effect Unit, router :: JSX, signal :: Signal (Route route) }
```
